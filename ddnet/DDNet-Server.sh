...
if [[ -n "$SV_IPV4ONLY" ]] ; then sed -i s/'sv_ipv4_only.*'/"sv_ipv4_only $SV_IPV4ONLY"/ "$CFG" ; fi
...
if [[ -n "$SV_REGISTER_URL" ]] ; then sed -i s/'sv_register_url.*'/"sv_register_url $SV_REGISTER_URL"/ "$CFG" ; fi
if [[ -n "$SV_SPECTATOR_SLOTS" ]] ; then sed -i s/'sv_spectator_slots.*'/"sv_spectator_slots $SV_SPECTATOR_SLOTS"/ "$CFG" ; fi
if [[ -n "$SV_TEST_CMDS" ]] ; then sed -i s/'sv_test_cmds.*'/"sv_test_cmds $SV_TEST_CMDS"/ "$CFG" ; fi
...
if [[ "$BANLIST" == "true" ]] ; then
	...
	if [[ -e "$BANLIST_PATH" ]] ; then
		...
		#/ddnet/DDNet-Server -f /root/.teeworlds/banlist
		/ddnet/DDNet-Server -f /root/.local/share/ddnet/banlist
	else
